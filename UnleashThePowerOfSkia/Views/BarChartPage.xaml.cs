...
namespace UnleashThePowerOfSkia.Views
{
    ...
    public partial class BarChartPage : ContentPage
    {
        ...
        private void DrawData(SKCanvas canvas)
        {
            ...
            using (var path = new SKPath())
            {
                var chartColors = new[] {SKColors.Magenta, SKColors.Navy, SKColors.Beige, SKColors.Aquamarine, SKColors.Gray, SKColors.Green, SKColors.Yellow};
                
                var lastDayValue = 0;
                SKColors lastChartColour;
                foreach (var averageSales in ViewModel.AverageSales)
                {
                    var randomColorIndex = new Random().Next(1, chartColors.Length);

                    var averageSalesRelative = averageSales.Value * valueScaleFactor;
                    using (var paint = new SKPaint {Color = chartColors[randomColorIndex], IsAntialias = true})
                    {
